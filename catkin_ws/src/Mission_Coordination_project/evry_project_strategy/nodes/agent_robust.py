...
import evry_project_plugins.srv
from evry_project_plugins.srv import DistanceToFlag
from evry_project_plugins.srv import DistanceToFlagRequest
...
class Robot:
    ...
    def getDistanceToFlag(self):
        ...
        try:
            ...
            return result.distance, pose
        except rospy.ServiceException as e:
            print("Service call failed: %s" % e)

    def move_to_flag(self, flag_x, flag_y):
        # Desired linear and angular velocities
        velocity = 1.0
        angle = 0.0

        # Compute the distance between the robot and the flag
        dx = flag_x - self.x
        dy = flag_y - self.y

        distance = math.sqrt(dx**2 + dy**2)

        # Use a proportional controller to set the linear velocity
        kp = 0.5  # Proportional gain
        velocity = kp * distance

        # Compute the angle to the flag
        #dx = flag_x - self.x
        #dy = flag_y - self.y
        desired_angle = math.atan2(dy, dx)

        # Use a proportional controller to set the angular velocity
        angular_error = desired_angle - self.yaw
        angle = kp * angular_error

        # Send the velocities to the robot
        self.set_speed_angle(velocity, angle)

        if distance >= 0 and distance <= 0.9:
            #print(f"{robot_name} Flag is reached = ", distance)
...
def run_demo():
    ...
    delay = 5
    # delay2 = 1
    rospy.sleep(delay+2*int(robot_name[-1]))
    # ospy.sleep(delay2+int(robot_name[1]))


# While something is detected by the sonar set the velovity to 0
    while not rospy.is_shutdown():
        ...
        distance, pose = robot.getDistanceToFlag()
        ...
        if(robot.robot_name == 'robot_1'):
            robot.move_to_flag(-7.21320344, 12.21320344)
        if(robot.robot_name == 'robot_2'):
            robot.move_to_flag(11.21320344, 20.21320344)

        if(robot.robot_name == 'robot_3'):
            robot.move_to_flag(10, -30)

        # robot2.move_to_flag(21.21320344,21.21320344)
        # robot3.move_to_flag(0,-30)

        #robot.set_speed_angle(velocity, angle)
        # robot.move_to_flag(flag_id)

       #robot.move_to_flag(-17.21320344,  20.21320344)
        ...
        # print(f"{robot_name} Flag is reached = ", distance)
        # robot.set_speed_angle(0, 0)
        # elif distance >0.9 and distance <= 60:
        # robot.set_speed_angle(2, 0)

       # else:
        # Finishing by publishing the desired speed.
        #    robot.move_to_flag(-21.21320344, 21.21320344)
